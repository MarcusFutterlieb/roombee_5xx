...
go_to_M     = [space_between,90];
write_M     = [h_start_end,-135;...
               c1,90;...
               c1,-135; ...
               h,90];

go_to_A     = [space_between,45];
write_A     = [c3,-90;      ...
               c3/3*2,-135; ...
               0.25,180;    ...
               0.25,-45;    ...
...
go_to_end   = [space_between,0];


target_vec =  [   turn_360;go_to_M; ... 
    write_M;go_to_A;   ...
    write_A;go_to_R;   ...
    write_R;go_to_E;   ...
    write_E;go_to_N;   ...
...
hold on;
ii = 1;
l_M    = size(turn_360,1) +     size(go_to_M,1) + size(write_M,1); 
l_A    = l_M +                  size(go_to_A,1) + size(write_A,1);
l_R    = l_A +                  size(go_to_R,1) + size(write_R,1);
l_E    = l_R +                  size(go_to_E,1) + size(write_E,1);
l_N    = l_E +                  size(go_to_N,1) + size(write_N,1);
l_vec = [l_M;l_A;l_R;l_E;l_N];
...
for jj=1:4:size(robo_path,1)
    ...
    if(ii<=size(l_vec,1))
        if (jj>letter_count(l_vec(ii)))
            happy_birthday_str = [happy_birthday_str maren_str(ii) ];
            ii = ii + 1;
        end%if
    end%if
...
disp('roomba: maren_b_day.m executed succesfully');
pause(3.);
disp('roomba: sry girls, but the odometry is to bad to write both names in a row :(');
pause(3.);
